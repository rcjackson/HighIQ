...
def _fast_expand(complex_array, factor, num_per_block=200):
    ...
    for i in range(shp[2]):
        ...
        rep_array = cp.transpose(cp.tile(gpu_array, (factor, 1, 1)), [1, 2, 0])
        diff_array = cp.transpose(cp.tile(diff_array, (factor, 1, 1)), [1, 2, 0])
